 ...
 * si son consecutivos.
 *
 * El premio físico final NO se define acá: lo elige el dueño marcando una
...
export const RETO60 = {
  ...
  windowDays: 60,
  milestones: [
    { at: 6, kp: 200, message: "¡Una semana de constancia! 6 de 24." },
    { at: 12, kp: 300, message: "¡Mitad de camino! 12 de 24." },
    { at: 18, kp: 500, message: "Recta final: 18 de 24. Ya casi." },
    { at: 24, kp: 1000, message: "¡Completaste el Reto 60!" },
  ],
} as const;

export type Reto60State = "not_started" | "active" | "completed" | "expired";

export interface Reto60Status {
  state: Reto60State;
  active: boolean; // conveniencia: state === "active"
  completed: boolean;
...
export function buildReto60Status(
  ...
  const expired = !completed && daysLeft <= 0;

  let state: Reto60State;
