-- ============================================================
-- KINETIC — Gating de canjes por estado del socio
-- Correr en Oracle después de status_migration.sql.
-- ============================================================
-- Recrea redeem_reward sumando: un socio 'pending' no puede canjear.
-- Mantiene los chequeos anteriores (no comprar premios de trigger, etc.).
...
declare
  ...
  v_attempts int := 0;
begin
  select status into v_status from profiles where id = p_user_id for update;
  if v_status is null then
    raise exception 'Socio no encontrado';
  end if;
  if v_status <> 'active' then
    raise exception 'Tu cuenta está pendiente de aprobación';
  end if;
